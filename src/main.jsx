...
const docsVacios = { foto: false, pasaporte: false, ds160: false, pagoAsesoria: false };

const inicial = {
  asesor: '', nombre: '', telefono: '', email: '', tipoCliente: 'afiliado', tipoSolicitud: 'primeraVez',
  fedex: '', documentos: docsVacios, observacion: '', seguimiento: ''
};
...
function calcularCaso(data) {
  const tarifa = tarifas[data.tipoCliente]?.[data.tipoSolicitud];
  const adicionalRenovacion = data.tipoSolicitud === 'renovacion' ? 155000 : 0;
  const requiereDerechos = data.tipoSolicitud === 'primeraVez' || data.tipoSolicitud === 'renovacion';
  const fedex = data.tipoSolicitud === 'primeraVez' && data.fedex ? Number(data.fedex) : 0;
  const totalPesos = (tarifa || 0) + adicionalRenovacion + fedex;
  const completos = documentosBase.filter(d => data.documentos?.[d.id]).length;
...
function App() {
  ...
  const [casos, setCasos] = useState([
    {
      ...
      total: 305000, estado: 'Documentos completos - Listo para agendar', documentos: '4/4',
      documentosObj: { foto: true, pasaporte: true, ds160: true, pagoAsesoria: true }, observacion: 'Pago validado. Lista para agendar asesoría.', seguimiento: 'Pendiente asignar horario de asesoría.'
    },
    {
      ...
      total: 190000, estado: 'Pendiente de documentos', documentos: '2/4',
      documentosObj: { foto: true, pasaporte: true, ds160: false, pagoAsesoria: false }, observacion: 'Falta DS-160 y soporte de pago.', seguimiento: 'Cliente enviará documentos pendientes.'
    },
  ]);
  ...
  function actualizarCaso(casoActualizado) {
    const calc = calcularCaso({
      tipoCliente: casoActualizado.tipoClienteKey,
      tipoSolicitud: casoActualizado.tipoSolicitudKey,
      fedex: casoActualizado.fedex || '',
      documentos: casoActualizado.documentosObj,
    });
    const actualizado = {
      ...casoActualizado,
      tipoCliente: tarifas[casoActualizado.tipoClienteKey].label,
      tipoSolicitud: tiposSolicitud.find(t => t.id === casoActualizado.tipoSolicitudKey)?.label,
      total: calc.totalPesos,
      estado: calc.estado,
      documentos: `${calc.completos}/4`,
    };
    setCasos(casos.map(c => c.id === actualizado.id ? actualizado : c));
  ...
  const titulo = vista === 'nuevoCaso' ? 'Nuevo caso de visa' : vista === 'casos' ? 'Casos registrados' : vista === 'detalleCaso' ? 'Detalle y seguimiento del caso' : vista === 'tarifas' ? 'Tarifas' : 'Dashboard';
  ...
  return <div className="app">
    ...
    <main className="content">
      <header><h1>{titulo}</h1><span>Fase 1.1 Web</span></header>
      ...
      {vista === 'nuevoCaso' && <form className="case-layout" onSubmit={guardarCaso}>
        <section className="panel">
          ...
          <Field required label="Email" value={form.email} onChange={v => setForm({...form, email:v})} />
          ...
          <Checklist documentos={form.documentos} onChange={(id, checked) => setForm({...form, documentos: {...form.documentos, [id]: checked}})} />
      ...
      {vista === 'casos' && <section className="panel">
        <table>
          <thead><tr><th>ID</th><th>Asesor</th><th>Cliente</th><th>Teléfono</th><th>Tipo</th><th>Solicitud</th><th>Documentos</th><th>Total</th><th>Estado</th><th>Acción</th></tr></thead>
          <tbody>{casos.map(c => <tr key={c.id}><td>{c.id}</td><td>{c.asesor}</td><td>{c.nombre}<br/><small>{c.email}</small></td><td>{c.telefono}</td><td>{c.tipoCliente}</td><td>{c.tipoSolicitud}</td><td>{c.documentos}</td><td>{moneda(c.total)}</td><td><span className={c.estado.includes('Pendiente')?'pill warn':'pill ok'}>{c.estado}</span></td><td><button className="small-btn" onClick={() => abrirCaso(c.id)}>Abrir</button></td></tr>)}</tbody>
        </table>
      </section>}
...
function DetalleCaso({ caso, onBack, onSave }) {
  ...
  const calc = calcularCaso({ tipoCliente: edit.tipoClienteKey, tipoSolicitud: edit.tipoSolicitudKey, fedex: edit.fedex || '', documentos: edit.documentosObj });
  ...
  return <div className="case-layout">
    <section className="panel">
      ...
      <div className="two-cols">
        ...
        <Field required label="Email" value={edit.email} onChange={v => setEdit({...edit, email:v})} />
      </div>
      ...
      <div className="two-cols">
        ...
        <label>Tipo de solicitud
          <select value={edit.tipoSolicitudKey} onChange={e => setEdit({...edit, tipoSolicitudKey:e.target.value})}>
            {tiposSolicitud.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
          </select>
        </label>
      </div>
      ...
      <Checklist documentos={edit.documentosObj} onChange={(id, checked) => setEdit({...edit, documentosObj: {...edit.documentosObj, [id]: checked}})} />
      <label>Observación general
        <textarea value={edit.observacion || ''} onChange={e => setEdit({...edit, observacion:e.target.value})} />
      </label>
      <label>Seguimiento del caso
        <textarea value={edit.seguimiento || ''} onChange={e => setEdit({...edit, seguimiento:e.target.value})} placeholder="Ej: se llamó al cliente, falta soporte, asesoría agendada..." />
      </label>
      <button className="primary" onClick={guardar}>Guardar cambios</button>
    </section>
    <Resumen calculo={calc} />
  </div>;
}

function Checklist({ documentos, onChange }) {
  return <div className="checklist">
    {documentosBase.map(d => <label key={d.id} className="check-item">
      <input type="checkbox" checked={!!documentos[d.id]} onChange={e => onChange(d.id, e.target.checked)} />
      <span>{d.label}</span>
    </label>)}
  </div>;
}

function Resumen({ calculo }) {
...
function Field({ label, value, onChange, required=false }) { return <label>{label}{required && <span className="required">Obligatorio</span>}<input value={value} onChange={e => onChange(e.target.value)} /></label>; }
